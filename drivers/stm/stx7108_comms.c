...
/*LPM external SBC resources I2C
static struct platform_device stx7108_lpm_device = {
	.name = "stm-lpm",
	.id = 0,
	.dev.platform_data = &(struct stm_plat_lpm_data){
	.number_i2c = 0,
	.number_gpio = 0,
	},
};*/
...
static struct i2c_board_info info = {
		...
		.platform_data = &(struct stm_plat_lpm_data){
		.number_i2c = 0,
		.number_gpio = 0,
		},
};

void  __init stx7108_configure_lpm(struct stm_plat_lpm_data *board_info)
{
	struct stm_plat_lpm_data *plat_data =
			info.platform_data;
	plat_data->number_i2c = board_info->number_i2c;
	plat_data->number_gpio = board_info->number_gpio;
	/*Not able to  add new device here */
	/* i2c_new_device(i2c_get_adapter(number_i2c), &info); */
}

struct i2c_board_info *stx7108_get_lpm_i2c(void)
{
	return &info;
}
...
static struct stm_plat_pwm_data stx7108_pwm_platform_data = {
	...
	.capt_pad_config = {
		[0] = &(struct stm_pad_config) {
			.gpios_num = 1,
			.gpios = (struct stm_pad_gpio []) {
				STM_PAD_PIO_IN(26, 6, 1),
			},
		},
		[1] = &(struct stm_pad_config) {
			.gpios_num = 1,
			.gpios = (struct stm_pad_gpio []) {
				STM_PAD_PIO_IN(14, 3, 1),
			},
		},
	},
};
...
void __init stx7108_configure_pwm(struct stx7108_pwm_config *config)
{
	...
	for (channel = 0; channel < ARRAY_SIZE(config->out_channel_config);
	     channel++)
		stx7108_pwm_platform_data.out_channel_config[channel] =
			config->out_channel_config[channel];

	for (channel = 0; channel < ARRAY_SIZE(config->capt_channel_config);
	     channel++)
		stx7108_pwm_platform_data.capt_channel_config[channel] =
			config->capt_channel_config[channel];
