...
static int clkgenb_set_parent(clk_t *clk_p, clk_t *parent_p)
{
	...
	unsigned long old_source, new_source;
	...
	/* If we changed the Clock Source from CLK_B_VID_HD_LOCAL to CLK_B_VID_SD_REMOTE, 
	or vice versa, we must pass through CLK_B_VID_SD_LOCAL to avoid lockup. */
	old_source = ((data  >> (chan * 2)) & 0x3);
	new_source = val;

	if((old_source == 0) && (new_source == 3))
	{
		data &= ~(0x3 << (chan * 2));
		data |= (1 << (chan * 2));
		SYSCONF_WRITE(0, 464, 0, 31, data);
	}
	else if((old_source == 3) && (new_source == 0))
	{
		data &= ~(0x3 << (chan * 2));
		data |= (1 << (chan * 2));
		SYSCONF_WRITE(0, 464, 0, 31, data);
...
static int clkgenc_enable(clk_t *clk_p)
{
	...
	case CLK_C_REF:
		return 0;
	case CLK_C_FS_VCO:
		/* PLL power-up  (NPDA) and rate recalc */
		SYSCONF_WRITE(0, 406, 14, 14, 1);
		return clkgenc_recalc(clk_p);
...
static int clkgenc_disable(clk_t *clk_p)
{
	...
	case CLK_C_REF:
		return 0;
	case CLK_C_FS_VCO:
		/* PLL power-down  (NPDA) and rate recalc */
		SYSCONF_WRITE(0, 406, 14, 14, 0);
		return clkgenc_recalc(clk_p);
	case CLK_C_SPDIF ... CLK_C_PCM1:
