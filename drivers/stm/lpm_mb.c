			...
			u16 *validsize, u16 datasize, char *data)
{
	...
	struct lpm_message response = {0};
	msg[0] = *wakeupdevice;
	msg[1] = (*wakeupdevice & 0xFF00)>>8;

	/* Copy size requested */
	put_unaligned_le16(datasize, &msg[2]);
	err = lpm_exchange_msg(&send_msg, &response);
	if (unlikely(err < 0))
		goto exit;
...
int stm_lpm_reset(enum stm_lpm_reset_type reset_type)
{
	...
	err = down_interruptible(&sbc_health_sem);
...
static void lpm_sbc_reply_worker(struct work_struct *work)
{
	...
	} else if (msg_p->command_id == LPM_MSG_INFORM_HOST) {
		if (msg_p->msg_data[0] == STM_LPM_LONG_FP_PIO_PRESS){
			int err = 0;
			if (lpm_drv->user_fn != NULL) {
				...
				if (err >= 0) {
					memcpy(&msg[2], &err, 4);
					msg_size = 6;
					msg_id = LPM_MSG_INFORM_HOST | LPM_MSG_REPLY;
				} else {
		...
		struct lpm_message *response)
{
	...
	if (lpm_msg.command_id & LPM_MSG_REPLY || send_msg->trans_id )
...
int stm_lpm_config_reboot(enum stm_lpm_config_reboot_type type)
{
	struct stm_lpm_adv_feature feature;
	u8 enabled = 1;

        feature.feature_name = STM_LPM_SET_EXIT_CPS;

	switch (type) {
	case stm_lpm_reboot_with_ddr_self_refresh:
		feature.params.set_params[0] = 0x9b ;
		break;
	case stm_lpm_reboot_with_ddr_off:
		feature.params.set_params[0] = 0xb9 ;
		break;
	default:
	...
	return stm_lpm_set_adv_feature(enabled, &feature);
}
...
static void lpm_reset_and_reload_firmware(int reload_sbc_fw)
{
	int i, err;
	...
	{
		struct ELF64_info *elfinfo = NULL;
		request_firmware(&sbc_fw,"lpm-fw-STxH205.elf",
		...
		if (unlikely(elfinfo == NULL))
        	...
        	feature.feature_name = STM_LPM_SBC_IDLE;
	...
	sysconf_write(lpm_drv->sc, 0x3);
	mdelay(10);
	sysconf_write(lpm_drv->sc, 0x1);
	...
	i = 0;
		...
		mdelay(100);
		err = stm_lpm_get_fw_state(&lpm_drv->sbc_state);
		if (err < 0 || lpm_drv->sbc_state == STM_LPM_SBC_RUNNING)
                        break;
                i++;
        } while (i != 10);
        if (err == 0) {
...
static int __init stm_lpm_probe(struct platform_device *pdev)
{
	...
	if (devm_request_irq(&pdev->dev, res->start, lpm_isr,
    	...
    	lpm_drv->sc = sysconf_claim(4, 0, 0, 31, "LPM_SW_START");
	BUG_ON(!lpm_drv->sc);
	...
	lpm_write32(lpm_drv, 1, MBX_INT_SET_ENABLE, 0xFF);
	lpm_drv->sbc_state = STM_LPM_SBC_BOOT;
	lpm_drv->lpm_pdev = kmalloc(sizeof(struct platform_device), GFP_KERNEL);
	memcpy(lpm_drv->lpm_pdev, pdev,sizeof(struct platform_device) );
