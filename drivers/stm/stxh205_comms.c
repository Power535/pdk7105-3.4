...
#ifdef CONFIG_SH_ST_B2141_SSC1_WITH_INTERNAL_PULL_UP
static struct stm_pad_config stxh205_ssc1_internal_pullup = {
	.sysconfs_num = 1,
	.sysconfs = (struct stm_pad_sysconf []) {
		STM_PAD_SYSCONF(SYSCONF(505), 14, 14, 0x0),
	},
	.gpios_num = 2,
	.gpios = (struct stm_pad_gpio []) {
		[0] = {
			.gpio = stm_gpio(12, 0),
			.function = 1, /* alternate 1 */
...
#ifdef CONFIG_SH_ST_B2141_SSC1_WITH_INTERNAL_PULL_UP
	if (ssc == 1) {
		pr_info("ssc-1 uses internal pull-up resistor\n");
		return &stxh205_ssc1_internal_pullup;
	}
#endif
...
int __init stxh205_configure_ssc_i2c(int ssc, struct stxh205_ssc_config *config)
{
	...
	if (ssc > 3)
		clk_add_alias_platform_device(NULL, &stxh205_ssc_devices[ssc], "sbc_comms_clk", NULL);
...
static struct stm_plat_pwm_data stxh205_pwm_platform_data = {
	...
	.capt_pad_config = {
		[0] = &(struct stm_pad_config) {
			.gpios_num = 1,
			.gpios = (struct stm_pad_gpio []) {
				STM_PAD_PIO_IN(3, 2, 1),
			},
		},
		[1] = &(struct stm_pad_config) {
			.gpios_num = 1,
			.gpios = (struct stm_pad_gpio []) {
				STM_PAD_PIO_IN(2, 3, 4),
			},
		},
	},
};
...
void __init stxh205_configure_pwm(struct stxh205_pwm_config *config)
{
	...
	for (channel = 0; channel < ARRAY_SIZE(config->out_channel_config);
	     channel++)
		stxh205_pwm_platform_data.out_channel_config[channel] =
			config->out_channel_config[channel];

	for (channel = 0; channel < ARRAY_SIZE(config->capt_channel_config);
	     channel++)
		stxh205_pwm_platform_data.capt_channel_config[channel] =
