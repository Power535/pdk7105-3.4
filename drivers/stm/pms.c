...
static struct kobject *pms_kobj;
/*
...
static struct pms_object *pms_create_object(int type, void *data)
{
	...
	if (pms_find_object(type, data)) {
		pr_debug("stm: pms: object already registered\n");
		goto err_0;
	}
...
struct pms_object *pms_register_clock_n(char *name)
{
	...
	if (!clk) {
		pr_debug("stm: pms: Clock not declared\n");
		return NULL;
	}
	pr_debug("stm: pms: cmd_add_clk: '%s'\n", name);
...
struct pms_object *pms_register_device_n(char *name)
{
	...
	if (!bus_name) {
		pr_debug("stm: pms: Error on bus name\n");
		goto err_0;
	}
	dev_name = _strsep((char **)&loc_dev_path, " \n\t\0");
	if (!dev_name) {
		pr_debug("stm: pms: Error on dev name\n");
		goto err_0;
	}
	bus = find_bus(bus_name);
	if (!bus) {
		pr_debug("stm: pms: Bus not declared\n");
		goto err_0;
	}
	dev = bus_find_device_by_name(bus, NULL, dev_name);
	if (!dev) {
		pr_debug("stm: pms: Device not found\n");
		goto err_0;
	}
	kfree(tmp);
...
error:
	pr_debug("stm: pms: Error to register the state %s\n", name);
	kfree(state);
	return NULL;
}
 ...
 */
	pr_debug("stm: pms: New constraint created\n");
...
error_pms_set_current_states:
	pr_debug("stm: pms: Error in the sets of state required\n");
	kfree(buf0);
	kfree(new_active_states);
	return -EINVAL;
}
...
int pms_global_standby(enum pms_standby_e state)
{
	...
	case PMS_GLOBAL_MEMHIBERNATION:
		...
		break;
#endif
	case PMS_GLOBAL_SHUTDOWN:
		kernel_power_off();
		break;
	default:
		pr_err("stm: pms:  Error: in %s state 0x%x not supported\n",
		       __func__, state);
	}
	...
	while ((p = _strsep((char **)&buf, " \t\n"))) {
		...
		case cmd_add_clk_constr:{
			...
			if (clk_is_readonly(clk)) {
				pr_debug("stm: pms: Clock read only\n");
				continue;
			}
		...
		case cmd_add_cpu_constr:{
			...
			int cpu_id;
			unsigned long rate;
			struct pms_object *obj __maybe_unused;
