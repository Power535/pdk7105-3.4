...
struct stm_plat_ssc_data {
	struct stm_pad_config *pad_config;
	unsigned int i2c_speed; /* Bus speed in KHz */
};
...
struct stm_plat_pwm_out_channel_config {
	int enabled;
	u8 initial_value;
...
/* Private data for the PWM capture driver */
struct stm_plat_pwm_capt_channel_config {
	int enabled;
};

struct stm_plat_pwm_data {
	const char *clk;
	struct stm_pad_config *out_pad_config[STM_PLAT_PWM_NUM_CHANNELS];
	struct stm_plat_pwm_out_channel_config
		out_channel_config[STM_PLAT_PWM_NUM_CHANNELS];
	struct stm_pad_config *capt_pad_config[STM_PLAT_PWM_NUM_CHANNELS];
	struct stm_plat_pwm_capt_channel_config
		capt_channel_config[STM_PLAT_PWM_NUM_CHANNELS];
};
