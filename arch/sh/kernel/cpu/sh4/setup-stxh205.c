...
void stxh205_prepare_restart(void)
{
	...
	err = stm_lpm_config_reboot(stm_lpm_reboot_with_ddr_off);
	if(err<0) goto full_soc_reset;

	err=stm_lpm_reset(STM_LPM_HOST_RESET);
	while(err==0)
		cpu_sleep();

full_soc_reset :
	/* If SBC firmware is not responding or not loaded */
	/* Perform a complete SOC reset (including SBC) */
