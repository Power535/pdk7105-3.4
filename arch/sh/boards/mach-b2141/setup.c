...
#define B2141_POWER_PIO		stm_gpio(3,7) 
...
static int __init device_init(void)
{
	...
	gpio_request(B2141_POWER_PIO, "POWER_ON");
	gpio_direction_output(B2141_POWER_PIO, 0);
	...
	/*
	 * On STiH273xYD (Palma 2), it required to properly configure the
	 * SYSCONF_506[31] = 0 in order to have SSC1 as master to control the
	 * QAM IP through an I2C slave IP.
	 * Instead on STiH237xYD (Cardiff 2) this bit is reserved, so proper
	 * runtime checking is required on SoC variant.
	 */
	if (boot_cpu_data.type == CPU_STXH273) {

		qam_sc = sysconf_claim(SYSCONF(506), 31, 31, "demod_gui");
		if (qam_sc) {
			/* SSC1 as master to control QAM IP */
			sysconf_write(qam_sc, 0);
		}
	}
