/*
 * arch/sh/boards/mach-b2175/setup.c
 *
...
#define B2175_GPIO_POWER_ON_ETH		stm_gpio(3, 3)

static void __init b2175_setup(char **cmdline_p)
{
	printk(KERN_INFO "STMicroelectronics B2175 board initialisation\n");
...
static struct platform_device b2175_leds = {
	.name = "leds-gpio",
	.id = -1,
...
/* Serial Flash */
static struct stm_plat_spifsm_data b2175_serial_flash =  {
	.name		= "n25q256",
	.nr_parts	= 2,
...
/* NAND Flash */
static struct stm_nand_bank_data b2175_nand_flash = {
...
static int b2175_phy_reset(void *bus)
{
	/*
	 ...
	 */
	gpio_set_value(B2175_GPIO_POWER_ON_ETH, 0);
	mdelay(10);
	gpio_set_value(B2175_GPIO_POWER_ON_ETH, 1);
	mdelay(10);
...
static struct stmmac_mdio_bus_data stmmac_mdio_bus = {
	.phy_reset = b2175_phy_reset,
	.phy_mask = 0,
	.probed_phy_irq = ILC_IRQ(25), /* MDINT */
};

static struct platform_device *b2175_devices[] __initdata = {
	&b2175_leds,
};
...
static int __init device_init(void)
{
	 ...
	 * but it isn't... ;-) */
	gpio_request(B2175_GPIO_POWER_ON_ETH, "POWER_ON_ETH");
	gpio_direction_output(B2175_GPIO_POWER_ON_ETH, 0);
	...
	stxh205_configure_ssc_i2c(STXH205_SSC(2), &(struct stxh205_ssc_config) {
			.routing.ssc3.sclk = stxh205_ssc2_sclk_pio9_4,
			.routing.ssc3.mtsr = stxh205_ssc2_mtsr_pio9_5, });
	...
	stxh205_configure_nand(&(struct stm_nand_config) {
			...
			.nr_banks = 1,
			.banks = &b2175_nand_flash,
			.rbn.flex_connected = 1,
			.bch_ecc_cfg = BCH_ECC_CFG_AUTO});

	stxh205_configure_spifsm(&b2175_serial_flash);

	return platform_add_devices(b2175_devices,
			ARRAY_SIZE(b2175_devices));
}
arch_initcall(device_init);

struct sh_machine_vector mv_b2175 __initmv = {
	.mv_name = "b2175",
	.mv_setup = b2175_setup,
	.mv_nr_irqs = NR_IRQS,
};
...
static int b2175_board_freeze(struct stm_wakeup_devices *wkd)
{
...
static int b2175_board_defrost(struct stm_wakeup_devices *wkd)
{
	b2175_phy_reset(NULL);
	return 0;
}

static struct stm_hom_board b2175_hom = {
	.freeze = b2175_board_freeze,
	.restore = b2175_board_defrost,
};

static int __init b2175_hom_register(void)
{
	return stm_hom_board_register(&b2175_hom);
}

module_init(b2175_hom_register);
#endif
